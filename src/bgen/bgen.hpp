...
namespace genfile {
	namespace bgen {
        ...
        template< typename Setter >
        void parse_probability_data_prs(
            byte_t const* buffer,
            byte_t const* const end,
            Context const& context,
            Setter& setter
...
#error "unknown endianness"
		...
		namespace v11 {
            ...
            void parse_probability_data_prs(
                ...
                setter.initialise(context.number_of_samples);
                ...
                for ( uint32_t i = 0 ; i < context.number_of_samples ; ++i ) {
                    ...
                    buffer = read_little_endian_integer( buffer, end, &prob ) ;
                    double p1 = impl::convert_from_integer_representation( prob, probability_conversion_factor );
                    buffer = read_little_endian_integer( buffer, end, &prob ) ;
                    double p2 = impl::convert_from_integer_representation( prob, probability_conversion_factor );
                    buffer = read_little_endian_integer( buffer, end, &prob ) ;

                    setter.update_genotype_value(i, p1, p2);
                }
		...
		namespace v12{
            ...
            template< typename Setter, typename BitParser >
            void parse_probability_data_diploid_biallelic_prs(
                GenotypeDataBlock const& pack,
                BitParser valueConsumer,
                Context const& context,
                Setter& setter
                ...
                setter.initialise(pack.numberOfSamples);

                {
                    if( pack.phased ) {
                        for( uint32_t i = 0; i < pack.numberOfSamples; ++i, ++ploidy_p ) {
                            bool const missing = (*ploidy_p & 0x80) ;
                            int const sample_status = (true * 0x1) + (missing * 0x2);
...
//                            }
                            ...
                            for( uint32_t hap = 0; hap < 2; ++hap ) {
                                ...
                                switch( sample_status ) {
                                    case eIgnore: break ;
                                    case eSetAsMissing:
                                        setter.set_missing(i);
                                        break ;
                                    case eSetThisSample:
                                    setter.update_hap_value(i, value);
                                        break ;
                                }
                            }
                        }
                        ...
                        for( uint32_t i = 0; i < pack.numberOfSamples; ++i, ++ploidy_p ) {
                            ...
                            switch( sample_status ) {
                                ...
                                case eSetAsMissing:
                                    ...
                                    break ;
                                case eSetThisSample:
                                    setter.update_genotype_value(i, value1, value2);
                                    break ;
                            }
                        }
                    }
            ...
            template< typename Setter, typename BitParser >
            void parse_probability_data_general_prs(
                GenotypeDataBlock const& pack,
                BitParser valueConsumer,
                Context const& context,
                Setter& setter
...

                {
                    if( pack.phased ) {
                        ...
                        for( uint32_t i = 0; i < pack.numberOfSamples; ++i, ++ploidy_p ) {
                            ...
                            if(storedValueCount<3){
                                for( uint32_t h = 0; h < storedValueCount; ++h ) {
                                    ...
                                    case eSetAsMissing:
                                        break ;
                                    case eSetThisSample:
                                        if(storedValueCount<3) p[h]=value;
                                        npassed+=1;
                                        break ;
                                    }
                                }
                            ...
                            if(storedValueCount==1 && npassed==1){
                                setter.update_ploidy1_genotype_value(i, p[0]);
                            }
                            else if(storedValueCount==2 && npassed==2){
                                setter.update_genotype_value(i, p[0], p[1]);
                            }
                            else{
                                setter.set_missing(i);
                            }
            ...
            void parse_probability_data_prs(
                ...
                if( pack.ploidyExtent[0] == 2 && pack.ploidyExtent[1] == 2 && pack.numberOfAlleles == 2 ) {
                    switch( pack.bits ) {
                        case 8:
                            parse_probability_data_diploid_biallelic_prs(
                                pack,
                                impl::SpecialisedBitParser<8>( pack.buffer, pack.end ),
                                context,
                                setter
                    ...
                    switch( pack.bits ) {
                        case 8:
                            parse_probability_data_general_prs(
                                pack,
                                impl::SpecialisedBitParser<8>( pack.buffer, pack.end ),
                                context,
                                setter
        ...
        void parse_probability_data_prs(
            ...
            if( (context.flags & e_Layout) == e_Layout0 || (context.flags & e_Layout) == e_Layout1 ) {
                v11::parse_probability_data_prs( buffer, end, context, setter ) ;
            } else {
                v12::parse_probability_data_prs( buffer, end, context, setter  ) ;
            }
        }
        ...
        template< typename Setter >
        void read_and_parse_genotype_data_block_prs(
            std::istream& aStream,
            Context const& context,
            Setter& setter,
            std::vector< byte_t >* buffer1,
