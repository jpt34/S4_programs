...
library(bigreadr)
library(tidyverse)
oncodata<-snp_attach("RefFiles/haplo_afr_hm3plus.rds")
G<-oncodata$genotypes
CHR<-as.integer(oncodata$map$chromosome)
sumstats = readRDS("RefFiles/breast_haplo_afr.rds")
NCORES=16
...
for (chr in 1:22) {
  ...
  corr0 <- readRDS(paste0("RefFiles/haplo_afr_correlation_chr", chr, ".rds"))[ind.chr3, ind.chr3]
...
write.table(beta_inf, "beta_breast_haplo_afr_inf_values.txt", row.names=FALSE, col.names=FALSE)
...
write.table(beta_grid, "beta_breast_haplo_afr_grid_values.txt", row.names=FALSE, col.names=FALSE)
