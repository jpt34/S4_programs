...
library(bigreadr)
library(tidyverse)
oncodata<-snp_attach("RefFiles/haplo_sas_hm3plus.rds")
G<-oncodata$genotypes
CHR<-as.integer(oncodata$map$chromosome)
sumstats = readRDS("RefFiles/t2d_haplo_sas.rds")
NCORES=16
...
for (chr in 1:22) {
  ...
  corr0 <- readRDS(paste0("RefFiles/haplo_sas_correlation_chr", chr, ".rds"))[ind.chr3, ind.chr3]
...
write.table(beta_inf, "beta_t2d_haplo_sas_inf_values.txt", row.names=FALSE, col.names=FALSE)
...
write.table(beta_grid, "beta_t2d_haplo_sas_grid_values.txt", row.names=FALSE, col.names=FALSE)
