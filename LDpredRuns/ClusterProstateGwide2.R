...
library(bigreadr)
library(tidyverse)
oncodata<-snp_attach("RefFiles/onco_ldpred_all.rds.rds")
G<-oncodata$genotypes
CHR<-as.integer(oncodata$map$chromosome)
sumstats = readRDS("RefFiles/prostate.rds")
NCORES=16
...
for (chr in 1:22) {
  ...
  corr0 <- readRDS(paste0("RefFiles/onco_correlation_chr", chr, ".rds"))[ind.chr3, ind.chr3]
...
write.table(beta_inf, "beta_prostate_inf_values2.txt", row.names=FALSE, col.names=FALSE)

(h2_seq <- round(h2_est * c(0.25, 0.35, 0.5), 4))
...
write.table(beta_grid, "beta_prostate_grid_values2_breast_onco.txt", row.names=FALSE, col.names=FALSE)
